...
namespace Bonanza.Storage.Benchmark
{
	...
	public class PgSqlEventStoreBenchmark
	{
		...
		public void SendManyEvents()
		{
			...
			var eventStore = new PostgreSql.PgSqlEventStore(connectionString, null, 0, AppendStrategy.OnePhase, true, false);
			...
			using (var enumerator = aggregatesIds.GetEnumerator())
			{
				while (enumerator.MoveNext() && eventsStored < EventsCount)
				{
					...
					eventStore.Append(aggregatesId.Key, data[(int)DataSize], aggregatesIds[aggregatesId.Key].Version, 0);
