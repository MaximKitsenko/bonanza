...
{
	/// <summary>
	/// Deals with serialization and naming in order to 
	...
	public class EventStore : IEventStore
	{
			...
			int take)
		{
			...
			foreach(var tapeRecord in records)
			{
				stream
					.Events
					.AddRange(DeserializeEvent(tapeRecord.Data));
		...
		{
			// in this project all identities producde proper name
			return id.ToString();
			...
			)
		{
			...
			catch(AppendOnlyStoreConcurrencyException e)
			{
				...
				throw OptimisticConcurrencyException.Create(
					server.Version,
					(long) e.ExpectedStreamVersion,
					id,
					server.Events
					);
			}

			// todo: there should be a parallel process that queries
			// new changes from the event store and send them via
			// messages (avoiding 2PC problem). however, for
			// demo purposes, we'll just send them to the console from here

			Console.ForegroundColor = ConsoleColor.DarkGreen;
			foreach(var @event in events)
			{
				Console.WriteLine(" {0} r{1} Event: {2}",
					id,
					originalVersion,
					@event);
			}
